...
**План разбивки (GSD, мелкие шаги):**
- [ ] Слой ингеста контента: таблица `content_sources` (тип, url/имя, статус, извлечённый текст) + парсеры: статья (URL→текст), PDF→текст. (YouTube-транскрипт — отдельным шагом, нужна доп. зависимость.)
- [ ] Чанкование+эмбеддинги загруженного контента — переиспользовать Phase 2 пайплайн (`indexing.py`).
- [ ] Генерация мини-курса (модули/уроки под уровень из фактов) + квизы.
- [ ] UI `/learn` (загрузка, курс, прогресс, тесты).
- [ ] `/security-review` (внешний контент = prompt-injection риск) + мерж.

**▶ ПЕРВЫЙ ШАГ:** создать ветку `phase-5-lecturer`, добавить миграцию `content_sources` + роут ингеста статьи/PDF (reuse чанкование). Доп. зависимости в `backend/pyproject.toml` (парсеры) — установить в `.venv`.
